...
const NextPrevButtons = ({index, total, prevHandler, nextHandler, classes, loop}) => {
  if (total < 2) return null
  const showNext = total > 1 && (!!loop || index < total - 1)
  const showPrev = total > 1 && (!!loop || index > 0)
  return <div className={classNames(classes.widget, classes.nextPrevWidget)}>
    <div>
      {showPrev && <IconButton onClick={prevHandler} className={classes.button}>
        <NavigateBeforeIcon />
      </IconButton>}
    </div>
    <div>
      {showNext && <IconButton onClick={nextHandler} className={classes.button}>
        <NavigateNextIcon />
      </IconButton>}
    </div>
  </div>
...
const Dots = ({index, total, prevHandler, nextHandler, classes, loop}) => (
  total > 1 && <div className={classNames(classes.widget, classes.dotsWidget)}>
    {Array(total).fill().map((_, i) => (
      <div key={i} className={classNames(classes.dot, {[classes.dotHighlight]: index === i})} />
    ))}
  </div>
)
